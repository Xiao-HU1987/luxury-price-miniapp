...
- [奢侈品比价小程序](#奢侈品比价小程序)
  ...
  - [开发规范](#开发规范)
    ...
    - [提交规范](#提交规范)
  - [常见问题](#常见问题)
    - [Q：小程序请求接口报 "request:fail timeout"？](#q小程序请求接口报-requestfail-timeout)
    - [Q：微信登录返回 mock 数据？](#q微信登录返回-mock-数据)
    - [Q：后台管理登录失败？](#q后台管理登录失败)
    - [Q：如何切换生产环境？](#q如何切换生产环境)
  - [许可证](#许可证)
...
| 门店管理 | 门店CRUD、类型/国家筛选 | ✅ |
| 汇率管理 | 汇率查询、手动更新 | ✅ |
| Trino 数据 | Trino 连通性、买手/门店/优惠券/订单读数据 | ✅ |
| 日志管理 | 访问日志、操作日志 | ✅ |
...
- **读数据层**：Trino（`localhost:8080`，后端读接口优先走 Trino）
...
后端已经内置 Trino 读接口，默认连接 `localhost:8080`。你可以在 [server/.env.example](server/.env.example) 中看到这些配置项：
...
后台管理新增了 [Trino 数据页](admin/src/views/TrinoData.vue)，用于查看连通性和买手、门店、优惠券、订单读数据。
