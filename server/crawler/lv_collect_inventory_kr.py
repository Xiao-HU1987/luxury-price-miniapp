...
logger = logging.getLogger("lv_inventory_kr")

BASE_DIR = Path(__file__).resolve().parent.parent.parent
DATA_DIR = BASE_DIR / "data" / "lv" / "KR"
...
# 节奏控制（降低 API 频率以避开风控阈值 ~110次/h）
REQUEST_DELAY = (15.0, 25.0)       # 同 SKU 内城市间延迟（目标 ~70次/h）
SKU_DELAY = (30.0, 60.0)           # SKU 间延迟
BATCH_SIZE = 3                      # 每 N 个 SKU 后长冷却
BATCH_COOLDOWN = (900, 1200)        # 批次冷却 15-20 分钟
CONSECUTIVE_403_LIMIT = 3           # 同 SKU 内连续 403 上限
RETRY_403_DELAY = (300, 600)        # 遇到 403 后冷却时间
API_TEST_RETRY_WAIT = [300, 600, 900]  # API 测试失败重试等待时间
GLOBAL_403_SKU_LIMIT = 5            # 连续 5 个 SKU 全 403 则退出


async def navigate_to_kr(page):
    """导航到 KR 首页，确保 cookies/localStorage 正确"""
    logger.info("导航到 KR 首页...")
    await page.goto("https://kr.louisvuitton.com/kor-kr/homepage",
...
async def extract_credentials(page) -> dict:
    ...
    device_id = await page.evaluate(
        "() => localStorage.getItem('LV.kor-kr.deviceId')"
    )
    ...
    result = await page.evaluate("""
        async ([sku, city, creds, xid]) => {
            ...
            const resp = await fetch(
                'https://api.louisvuitton.com/eco-eu/search-merch-eapi/v1/kor-kr/stores/query',
                {
                    method: 'POST',
                    credentials: 'include',
                    headers: headers,
...
async def query_availability(page, sku: str, creds: dict) -> dict:
    ...
    result = await page.evaluate("""
        async ([sku, creds, xid]) => {
            ...
            const resp = await fetch(
                'https://api.louisvuitton.com/eco-as/lvcom-prodct-dtl-eapi/v2/products/kor-kr/availability',
                {
                    method: 'POST',
                    credentials: 'include',
                    headers: headers,
    ...
    for city in CITIES:
        ...
        if result["status"] == 403:
            ...
            if consecutive_403 >= CONSECUTIVE_403_LIMIT:
                logger.warning(f"  连续 {consecutive_403} 次 403，冷却后重试...")
                await asyncio.sleep(random.uniform(*RETRY_403_DELAY))
                consecutive_403 = 0
...
async def main():
    sku_file = DATA_DIR / "products_KR_clean.jsonl"
    output_path = DATA_DIR / "inventories_KR_api.jsonl"
    stores_path = DATA_DIR / "inventories_KR_stores.jsonl"
    MIN_STORES_THRESHOLD = 5  # KR 门店较 JP 少，降低阈值
    ...
    with open(sku_file, "r") as f:
        for line in f:
            try:
                ...
                pass
    logger.info(f"全部 KR SKU: {len(all_skus)} 个")
    ...
    if api_partial_skus and output_path.exists():
        ...
        if to_clean:
            logger.info(f"清理 API 部分完成的 SKU 数据: {len(to_clean)} 个")
            ...
            logger.info(f"已清理 {len(to_clean)} 个")
    ...
    async with async_playwright() as pw:
        ...
        for retry in range(3):
            test = await query_stores(page, pending[0], "서울", creds)
            if test["status"] == 200:
        ...
        # 逐个处理 SKU
        summaries = []
        consecutive_zero_skus = 0  # 连续全 403 的 SKU 计数
        for i, sku in enumerate(pending):
            logger.info(f"\n[{i+1}/{len(pending)}] {sku}")
            summary = await process_sku(page, sku, creds, output_path)
            summaries.append(summary)

            # 全局 403 检测：如果整个 SKU 没有拿到任何门店数据，计数
            if summary["total_stores"] == 0 and summary["cities_403"] > 0:
                consecutive_zero_skus += 1
                logger.warning(f"全 403 SKU 计数: {consecutive_zero_skus}/{GLOBAL_403_SKU_LIMIT}")
                if consecutive_zero_skus >= GLOBAL_403_SKU_LIMIT:
                    logger.error(f"连续 {GLOBAL_403_SKU_LIMIT} 个 SKU 全 403，"
                                 f"API 限流中，退出等待自然冷却后重试")
                    break
            else:
                consecutive_zero_skus = 0
